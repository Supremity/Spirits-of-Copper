...
@onready var input_country = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputCountry
@onready var input_country_color = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputCountryColor
@onready var input_city = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputCity
@onready var input_pop = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputPop
@onready var input_gdp = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputGDP
@onready var input_eth_name = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputEthName
@onready var input_eth_color = $PanelContainer/MarginContainer/ScrollContainer/VBoxContainer/GridContainer/InputEthColor
...
func select_province(pid: int) -> void:
	...
	if prov.ethnicity.has("name"): input_eth_name.text = prov.ethnicity["name"]
	if prov.ethnicity.has("color"): input_eth_color.text = prov.ethnicity["color"]
...
func _on_apply_pressed() -> void:
	...
	prov.ethnicity = { 
		"name": input_eth_name.text.strip_edges(), 
		"color": input_eth_color.text.strip_edges() 
	}
	...
	for c in raw_claims:
		...
		if clean != "": final_claims.append(clean)
...
func _on_export_pressed() -> void:
	...
	for pid in MapManager.province_objects.keys():
		...
		if p.r_color == null: continue
		var key = _color_to_rgb_string(p.r_color)
		prov_export[key] = {
			"city": p.city, "claims": p.claims, "ethnicity": p.ethnicity,
			"gdp": p.gdp, "population": p.population
		}
...
func _color_to_rgb_string(c: Color) -> String:
	return "(%d, %d, %d)" % [int(round(c.r * 255.0)), int(round(c.g * 255.0)), int(round(c.b * 255.0))]
